  ...
  return (
    <div className="py-2">
      <div className="flex items-center justify-between px-3 mb-2">
        <span className="text-xs font-medium text-gray-500 uppercase tracking-wide">Главы и сцены</span>
        <button onClick={handleCreateChapter} className="p-1 rounded hover:bg-gray-200 text-gray-500" title="Добавить главу">
          <Plus size={14} />
        </button>
      </div>
      ...
      {chapters.length === 0 && (
        <div className="px-3 py-4 text-center text-sm text-gray-400">
          ...
          <p>Пока нет глав</p>
          <button onClick={handleCreateChapter} className="mt-2 text-blue-600 hover:underline text-xs">Создать первую главу</button>
        </div>
      )}
      ...
      {chapters.map((chapter) => (
        <div key={chapter.id} className="mb-1">
          ...
          >
            ...
            <button onClick={(e) => { e.stopPropagation(); handleCreateScene(chapter.id); }} className="p-0.5 rounded hover:bg-gray-200 text-gray-400 opacity-0 group-hover:opacity-100" title="Добавить сцену">
              <Plus size={12} />
            </button>
          </div>
            ...
            <div className="ml-6">
              {chapter.scenes.map((scene) => (
                <div
                  ...
                  className={`flex items-center gap-2 px-3 py-1.5 text-sm rounded-md cursor-pointer ${scene.id === activeSceneId ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-600 hover:bg-gray-100'}`}
                >
              ...
              {chapter.scenes.length === 0 && <div className="px-3 py-2 text-xs text-gray-400">Нет сцен</div>}
            </div>
          )}
        </div>
      ))}
